...
const ResponseCard = ({ className }: { className?: string }) => {
  ...
  // Get user question text for context
  const getUserQuestionText = (conversation: Conversation | undefined) => {
    if (!conversation) return "";
    return conversation.user.parts
  ...
  // useEffect(() => setShowInDetail(false), [currentConversation]);
  useHotkeys(SHORTCUTS.PREVIOUS_CHAT, prev);
  useHotkeys(SHORTCUTS.NEXT_CHAT, next);
  useHotkeys(SHORTCUTS.IN_DETAIL, () => setShowInDetail((prev) => !prev));
  ...
  return (
    <>
      <motion.div
        ...
        className={cn("card-box mt-2 min-w-lg p-2 flex-col", className)}
      >
        {/* Response Actions */}
        {currentConversation && (
          <>
            ...
            <div className="mt-2">
              <div className="flex justify-end">
                {currentConversation.user.parts.map(
                    ...
                    (p: MessagePart, i: number) =>
                      p.type === "text" ? (
                        ...
                        >
                          {p.text}
                          <button
                            className="my-2 font-semibold text-white underline flex items-center justify-center"
                            onClick={() => setShowInDetail(true)}
                          >
                            Show In Detail <ChevronRight className="size-5" />
                          </button>
                        </div>
                      ) : null
                  )
        ...
        >
          <input
            className="text-xs w-full border-none outline-none font-semibold text-white"
            autoComplete="off"
            value={input}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type="submit"
                className="bg-primary/70 py-1 text-xs
               px-3 rounded-md"
              >
                Submit
              </button>
      ...
      {showInDetail && (
        <InDetail userQuestion={getUserQuestionText(currentConversation)} />
      )}
    </>
  );
};
